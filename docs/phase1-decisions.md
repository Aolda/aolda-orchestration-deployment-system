...
Phase 1 제품 계약은 GitHub + IIV + Kubernetes + Prometheus 연동을 포함한다.
하지만 **개발 환경에서의 실연동 순서**는 아래처럼 가져간다.

1. GitHub 기본 브랜치 direct push를 먼저 실연동한다.
2. IIV는 인터페이스와 경로 계약을 유지한 채 local adapter를 당분간 사용한다.
3. GitHub write/read 경로가 안정화된 뒤 IIV KV v2 호환 adapter를 붙인다.
...
* IIV 최종 경로 규칙 `secret/aods/apps/{projectId}/{appName}/prod` 는 지금부터 고정한다.
* local secret adapter는 dev fallback 일 뿐이며, 운영 연결 전에는 IIV adapter로 교체해야 한다.

## 5. IIV 경로 규칙

IIV는 KV v2 호환 API 기준으로 잡는다.
...
* `500`: GitHub, IIV, Prometheus, Kubernetes 연동 실패 또는 서버 내부 오류
