...
* 브랜치: `main`
* 시작 계약 문서는 여전히 historical Phase 1 문서군이다.
* 하지만 baseline 판단 기준은 **`docs/current-implementation-status.md` 에 정리된 post-phase current implementation baseline** 이다.
* 현재 코드베이스는 Phase 1/2/3/4 planning bucket 을 지난 통합 플랫폼 상태로 본다.
...
* `GET /api/v1/projects` 는 최소 계약에 포함된다.
* 권한 최소 모델은 `viewer`, `deployer`, `admin` 이다.
* Flux UI 상태는 `Unknown`, `Syncing`, `Synced`, `Degraded` 네 개만 쓴다.
* Progressive delivery, change review, environments, policy, auto rollback, cluster scale 은 현재 baseline 이다.
* `pull_request` write mode 는 현재 실제 GitHub PR 생성/머지와 동일한 뜻이 아니라 review-gated change flow 를 뜻한다.
* 일부 frontend UI 는 `frontend/src/app/appConfig.ts` feature flag 로 숨겨질 수 있다. API/코드 baseline 과 현재 노출 surface 를 구분한다.

이 결정은 문서로 고정돼 있다. 추측으로 바꾸지 않는다.

주의:

* 이 runbook 은 Phase 1 문서군을 historical minimum contract 로 삼지만, 실제 작업 우선순위는 current implementation baseline 회귀 방지 기준으로 잡아야 한다.
* 현재 레포를 phase 구현 진행 중 상태라고 가정하면 실제 코드 상태를 과소평가하게 된다.
* phase 진행상황 판단이 아니라 post-phase baseline 판단을 `docs/current-implementation-status.md` 와 실제 코드 기준으로 확인한다.
...
```text
AODS post-phase current implementation baseline을 사용자 개입 최소화로 유지·강화해라.
...
작업 목표:
- historical Phase 1 최소 계약을 깨지 않는다.
- 현재 코드와 UI에 이미 노출된 전체 기능을 regression 없이 유지한다.
...
- 사용자가 명시적으로 요청하지 않는 한 현재 baseline 밖의 완전히 새로운 기능은 임의로 열지 않는다.
...
- feature flag 로 숨겨진 UI 는 구현 상태와 노출 상태를 구분해서 설명한다.
...
export AODS_FLUX_STATUS_CACHE_TTL="10s"
...
SQL 기반 application catalog projection 과 MariaDB 기반 deployment operation queue 를 붙일 때는 아래 env 를 추가한다.

```bash
# PostgreSQL UI read model projection.
export AODS_APPLICATION_CATALOG_DSN='postgres://aods:password@postgres.aods-system.svc.cluster.local:5432/aolda?sslmode=disable'
export AODS_APPLICATION_CATALOG_DB_DRIVER=postgres
export AODS_MARIADB_DSN='aods_user:password@tcp(mariadb.aods-system.svc.cluster.local:3306)/aods'
export AODS_APPLICATION_CATALOG_CACHE_TTL=10m
export AODS_APPLICATION_CATALOG_SYNC_INTERVAL=60s
...
* `AODS_APPLICATION_CATALOG_DSN` 이 설정되면 앱 목록 API 는 PostgreSQL 또는 MariaDB projection 을 먼저 읽고, background projector 가 GitHub 기본 브랜치 내용을 주기적으로 DB에 반영한다.
* `AODS_APPLICATION_CATALOG_DB_DRIVER` 는 비워도 DSN scheme 으로 `postgres`/`mysql` 을 추론한다.
...
* PostgreSQL/MariaDB projection 은 UI read-model 이며, MariaDB operation queue 는 durable queue / 상태 전이 / Git write 직렬화 coordinator 이다. desired state 의 source of truth 는 계속 GitHub 기본 브랜치다.
