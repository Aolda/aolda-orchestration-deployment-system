...
* 이 문서는 여전히 **Phase 1 sign-off 기준선**이다.
* 현재 레포에는 Phase 2, 3, 4 일부 기능이 이미 들어와 있어도, 본 문서의 인수 범위는 Phase 1 계약 기준으로 유지한다.
* later-phase 구현 현황은 `docs/current-implementation-status.md` 를 함께 본다.
* 즉, 이 문서는 현재 제품의 전체 regression baseline 설명 문서가 아니다.
...
이 항목들은 Phase 2 이상 인수 조건에서 별도로 다룬다.

주의:

* 현재 코드에는 위 항목과 관련된 later-phase 구현이 일부 포함될 수 있다.
* 하지만 본 문서는 **Phase 1 완료 판정**에만 사용한다.
