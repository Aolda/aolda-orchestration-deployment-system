...
* 이 문서는 여전히 Phase 2, 3, 4의 **설계 방향**을 고정하는 문서다.
* 다만 현재 레포에는 이 문서의 일부 항목이 이미 선반영돼 있다.
* 따라서 roadmap 을 `아직 아무 것도 시작 안 한 일 목록`으로 읽으면 안 된다.
* 실제 구현 범위는 `docs/current-implementation-status.md` 와 코드를 함께 보고 판단한다.
* 즉, Phase 표기는 현재 제품 baseline 을 가리키는 active gate 가 아니라, 남은 공백과 설계 축을 설명하는 historical planning bucket 이다.
...
즉, 앞으로 기능이 늘어나도 지금 정한 최소 계약을 뒤엎지 않는다.
...
현재 코드에는 아래 항목이 이미 들어와 있다.

* `Canary` 전략 허용
* deployment history / deployment detail 조회
* `Promote`
* `Abort`
* rollout 상태 필드와 프론트 표시

즉, Phase 2는 현재 레포에서 `설계만 존재`하는 단계가 아니라, 이미 상당 부분 구현된 상태다.
...
현재 코드에는 아래 항목이 부분적으로 들어와 있다.

* environments
* repositories 조회
* `Change` 생성과 submit / approve / merge 상태 전이
* diff preview 데이터
* `admin` 승인 권한 분리
* 환경 전환 redeploy
...
* 현재 `pull_request` 는 **실제 GitHub Pull Request 생성/리뷰/머지 흐름이 아니다.**
* 현재 구현은 review-gated direct apply 에 가깝다.
* 따라서 이 phase 의 `PR 기반 write mode` 는 아직 제품 목표로만 열어두고, 현재 구현 완료로 적지 않는다.
...
* cluster catalog 와 cluster target 연결
* platform admin 이 cluster catalog 와 Flux bootstrap scaffold 를 생성하는 cluster bootstrap API
* 프로젝트 정책 조회/수정과 환경/전략/클러스터 타겟 enforcement
* rollback policy 저장/조회
* application events 저장/조회
* cluster-aware Flux bootstrap / child kustomization wiring
* poller 기반 메트릭 평가 후 자동 재배포를 수행하는 auto rollback executor
* stale staging secret 을 정리하는 cleanup worker
* AODS 가 관리하는 orphan Flux child manifest 를 정리하는 cleanup worker

아직 남은 항목:

* project catalog 까지 포함하는 full self-service bootstrap 운영 흐름
* delete/archive 와 연결된 full lifecycle cleanup
* 더 정교한 다중 클러스터 운영 UX
