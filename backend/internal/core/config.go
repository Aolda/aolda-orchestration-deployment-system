...
func LoadConfig() (Config, error) {
	...
	deploymentOperationInterval, err := envDuration("AODS_DEPLOYMENT_OPERATION_INTERVAL", 2*time.Second)
	if err != nil {
		return Config{}, err
	}

	deploymentOperationLease, err := envDuration("AODS_DEPLOYMENT_OPERATION_LEASE", 5*time.Minute)
	if err != nil {
		return Config{}, err
	}

	deploymentOperationMaxAttempts, err := envInt("AODS_DEPLOYMENT_OPERATION_MAX_ATTEMPTS", 5)
	if err != nil {
		return Config{}, err
	}
